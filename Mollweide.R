...
target_crs_mollweide <- "+proj=moll +datum=WGS84"

# Download and prepare basemap data --------------------------------------
world_countries <- ne_countries(scale = 'medium', returnclass = 'sf')
world_oceans <- ne_download(scale = 'medium', type = 'ocean', 
                            category = 'physical', returnclass = 'sf')
world_countries_mollweide <- st_transform(world_countries, crs = target_crs_mollweide)
world_oceans_mollweide <- st_transform(world_oceans, crs = target_crs_mollweide)


# Create graticules -------------------------------------------------------
graticules_mollweide <- st_graticule(
  lat = seq(-90, 90, by = 10),
  crs = st_crs(4326)
) |> st_transform(crs = target_crs_mollweide)
...
# Create BASE MAP (Mollweide) --------------------------------------------
map <- ggplot() +
  geom_sf(data = world_oceans_mollweide, fill = ocean_color, color = NA) +
  geom_sf(data = world_countries_mollweide, fill = country_color,
          color = country_border_color, linewidth = 0.2) +
  geom_sf(data = graticules_mollweide, color = "white", linewidth = 0.4, alpha = 0.4) +
