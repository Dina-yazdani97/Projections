...
# Create BASE MAP (Robinson only) ----------------------------------------
map <- ggplot() +
  geom_sf(data = world_oceans_robinson, fill = ocean_color, color = NA) +

  geom_sf(data = world_countries_robinson, fill = country_color,
          color = country_border_color, linewidth = 0.5) +

  geom_sf(data = graticules_robinson, color = "white", linewidth = 0.3, alpha = 0.3) +
...
ggsave(paste0("Robinson",  "map.png"), 
